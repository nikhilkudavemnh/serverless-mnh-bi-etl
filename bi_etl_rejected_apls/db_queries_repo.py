SELECT_QUERIES_REPOSITORY = {
    ...
    "etl_queries": {
        "get_data": "select appln.opening_id as rid, appln.app_id as apl_id, s.status_name as from_step, ae.exit_code as rejection_code, ae.exit_reason_category as rejection_reason, to_char(appln.last_status_change_date,'YYYY-MM-DD') as rejected_on, o.bu_id from application appln join application_exit ae on ae.application_id = appln.app_id join opening o on o.opening_id = appln.opening_id join status s on s.status_id = ae.from_step where appln.status_id =13 and appln.app_id in {idsFilter};"
}}
...
UPDATE_QUERIES_REPOSITORY = {
    "insert_update_query": """INSERT INTO {schema_name}.rejected_apls ({columns}) VALUES (%s, %s, %s, %s, %s, %s, %s)
