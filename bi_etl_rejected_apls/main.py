...
def load_data(schema_name, transformed_data):
    try:
        ...
        logger.error(f"Error loading rejected_apls data in database: {e}")
        raise Exception(f"Error loading rejected_apls data in database: {e}")
...
def etl_runner(src_schema_name: str, dst_schema_name: str):
    try:
        ...
        logger.error(f"Error running rejected_apls ETL: {e}")
        raise Exception(f"Error running rejected_apls ETL: {e}")
